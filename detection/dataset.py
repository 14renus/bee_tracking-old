...
Params:
  ...
  a: angle in radians
  xd: x total dimension (normally, height of image)
  yd: y total dimension (normally, width of image)
  r1: height (?) of ellipse
  r2: width (?) of ellipse
'''
def ellipse_around_point(xc, yc, a, xd, yd, r1, r2):
    ind = np.zeros((2, xd, yd), dtype=np.int)
    m = np.zeros((xd, yd), dtype=np.float32)
    for i in range(yd):
        ind[0,:,i] = range(-yc, xd-yc)
    for i in range(xd):
        ind[1,i,:] = range(-xc, yd-xc)
...
Params
  ...
  pos_dir: directory holding .txt files with the same frame number format.
     Each row represents a bee object with 4 position args: x center, y center, bee class (0 for full/1 for cell), angle in degrees
